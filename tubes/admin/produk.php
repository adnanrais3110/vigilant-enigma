...
//pagination
$banyaksabun = 7;
$totalsabun = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM data_sabun_muka"));
...
$listsabun = query("SELECT * FROM data_sabun_muka LIMIT $sabunawal, $banyaksabun");
//end

//fitur search
if (isset($_POST['cari'])) {
  $keyword = $_POST['keyword'];
  $listsabun = query("SELECT * FROM data_sabun_muka WHERE 
        nama_sabun_muka LIKE '%$keyword%' OR 
        bahan_sabun_muka LIKE '%$keyword%' OR 
        kegunaan_sabun_muka LIKE '%$keyword%' OR
        harga_sabun_muka LIKE '%$keyword%'
...
<body>
    ...
    <div class="px-3 py-2 bg-info text-dark fs-4">
      <div class="container">
        <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
          <a href="/" class="d-flex align-items-center my-2 my-lg-0 me-lg-auto text-dark text-decoration-none"><i class="bi bi-droplet-half"></i>AdnanRise
          </a>
  ...
  <div class="container-xl mt-5 fs-4">
    ...
    <p>Oleh AdnanRise personal group</p>
    <div class="row">
      <div class="col-md-7">
        <h4></h4>
      </div>
    ...
    <table border="1" cellpadding="10" cellspacing="0" class="table table-bordered table-hover table-secondary mt-4">
      ...
      <?php foreach ($listsabun as $row) : ?>
        <tr class="thead-dark text-center">
          <td><?= $i; ?></td>
          <td><img src="../asset/uploaded-img/<?= $row["gambar_sabun_muka"]; ?>" class="rounded foto" width="auto" height="50px"></td>
          <td><?= $row["nama_sabun_muka"]; ?></td>
          <td><?= $row["bahan_sabun_muka"]; ?></td>
          <td><?= $row["kegunaan_sabun_muka"]; ?></td>
          <td><?= $row["harga_sabun_muka"]; ?></td>
          <td>
            <a href="ubah.php?id_sabun_muka=<?= $row["id_sabun_muka"]; ?>" class="btn btn-success">Ubah</a>
            <a href="hapus.php?id_sabun_muka=<?= $row["id_sabun_muka"]; ?> " class="btn btn-warning" onclick="return confirm('Hapus data?');">hapus</a>
