...
if(isset($_POST['order_btn'])){
   ...
   if(mysqli_num_rows($cart_query) > 0){
      while($product_item = mysqli_fetch_assoc($cart_query)){
         $product_name[] = $product_item['nama_sabun_muka_cart'] .' ('. $product_item['quantity'] .') ';
         $product_price = ($product_item['harga_sabun_muka_cart'] * $product_item['quantity']);
   ...
   <div class="display-order">
      <?php
         ...
         if(mysqli_num_rows($select_cart) > 0){
            ...
            $total_price = $fetch_cart['harga_sabun_muka_cart'] * $fetch_cart['quantity'];
            $tp = number_format($total_price);
            $grand_total = $total += $total_price;
      ?>
      <span><?= $fetch_cart['nama_sabun_muka_cart']; ?>(<?= $fetch_cart['quantity']; ?>)
         <br><img src="<?= $fetch_cart['gambar_sabun_muka_cart']; ?>" width="auto" height="50px">
      </span>
      <?php
         }
      }else{
